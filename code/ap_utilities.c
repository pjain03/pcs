...
void extract_keywords(HTTPResponse **response) {
    ...
    len = strlen(clean_body); // This should be fine, since no special characters in this string
    ...
    while (curr_word != NULL) {
        ...
        if (word_len > 2) { // Only store words that are more than two character long
            ...
            if(!is_stop_word(stop_words, curr_word)) { // Only continue if it is not a stop word. Ignore stop words
                ...
                if (curr == NULL) {
                    ...
                    curr->word[word_len] = '\0';                    curr->count = 1; 
